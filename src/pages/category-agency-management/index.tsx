...
const AgencyCategoryListPage = () => {
    ...
    const onDeleteAsync = async () => {
        try {
            await categoryAgencyService.DeleteAgencyCategoryAdmin(
                idSelected,
                setLoading
            ).then((res) => {
                if (res) {
    ...
    const onNavigate = (id: any) => {
        router(`${(ROUTE_PATH.VIEW_CATEGORY_AGENCY_MANAGEMENT).replace(`${Constants.UseParams.Id}`, "")}${id}`);
    }
        ...
        >
            <div className={styles.manage_container}>
                ...
                <div className={styles.searchBar}>
                    ...
                    />
                    <ButtonHref
                        href={ROUTE_PATH.ADD_CATEGORY_AGENCY_MANAGEMENT}
                        title={'Thêm mới'}
                        width={150}
                ...
                <DialogConfirmCommon
                    message={"Bạn có muốn xóa dòng sản phẩm này ra khỏi hệ thống"}
                    titleCancel={"Bỏ qua"}
                    titleOk={"Xóa"}
...
export default AgencyCategoryListPage;